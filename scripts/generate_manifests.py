...
MANIFESTS = {
    'MANIFEST_ROOT.json': {'dirs': ['.github'], 'files': []},
    'MANIFEST_BACKEND.json': {'dirs': ['python_service', 'pg_schemas', 'tests'],
 'files': []},
...
def main():
    ...
    for manifest_name, criteria in MANIFESTS.items():
        manifest_files = []
        for f in all_files:
            # Check if the file is within one of the target directories
            # or if it's a direct child of a target directory
            in_dir = any(f.startswith(d + '/') for d in criteria['dirs'])
            is_child = any(f == d for d in criteria['dirs'] if '/' not in f)

            if in_dir or (criteria['dirs'] and is_child):
    ...
    # Manually add critical missing files if they weren't caught
    if 'tests/conftest.py' in all_files and 'tests/conftest.py' not in MANIFESTS['MANIFEST_BACKEND.json']['files']:
        MANIFESTS['MANIFEST_BACKEND.json']['files'].append('tests/conftest.py')
    if 'python_service/requirements.txt' in all_files and 'python_service/requirements.txt' not in MANIFESTS['MANIFEST_BACKEND.json']['files']:
        MANIFESTS['MANIFEST_BACKEND.json']['files'].append('python_service/requirements.txt')

    MANIFESTS['MANIFEST_BACKEND.json']['files'].sort()
