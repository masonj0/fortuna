...
class ConnectionManager:
    ...
    async def broadcast(self, message: dict):
        if not self.active_connections:
            return
        log.info("Broadcasting message", client_count=len(self.active_connections))
...
async def lifespan(app: FastAPI):
    ...
    log.info("Heavy resource initialization scheduled.")
    yield
...
def create_app(settings=None) -> FastAPI:
    ...
    # Pass the limiter instance to the routing function
    define_routes(app, limiter)

    return app

def define_routes(app: FastAPI, limiter: Limiter):
