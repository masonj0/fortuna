...
# --- Web Service Mode: Serve Static Frontend ---
# When running as a packaged web service, the backend is also responsible
# for serving the static frontend files. We check an environment variable
# to enable this mode.
if os.getenv("FORTUNA_MODE") == "webservice":
    log.info("FORTUNA_MODE is 'webservice', serving static frontend files.")

    # Determine the path to the static files. When running as a PyInstaller
    # bundle, files are in a temporary folder accessible via sys._MEIPASS.
    if getattr(sys, 'frozen', False):
        static_files_dir = os.path.join(sys._MEIPASS, 'ui')
        log.info(f"Running in frozen mode, serving from: {static_files_dir}")
    else:
        # For local development of the web service, we point to a conventional path.
        # This requires the frontend to be built into 'web_service/frontend/out'.
        static_files_dir = "web_service/frontend/out"
        log.info(f"Running in development mode, serving from: {static_files_dir}")
